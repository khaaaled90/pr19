...
class _KeywordsScreenState extends State<KeywordsScreen> {
  ...
  Future<void> _notifyNativeToReloadCache() async {
    try {
      await _controlChannel.invokeMethod('clearCache');
      debugPrint("✅ Native cache cleared successfully.");
    } on PlatformException catch (e) {
      debugPrint("❌ Failed to clear native cache: '${e.message}'.");
    }
  }
  ...
  void _showKeywordDialog({Map<String, dynamic>? keywordToEdit}) {
    ...
    final TextEditingController priceController = TextEditingController(
      text: isEditing ? (keywordToEdit['price'] ?? 0.0).toString() : '0',
    );
    ...
    // 🟢 إضافة متغير فحص هل الباقة مجانية
    bool isFree = isEditing 
        ? ((keywordToEdit['price'] ?? 0.0) == 0.0 || keywordToEdit['keyword'].toString().toLowerCase().contains('f'))
        : false;
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            // 🟢 تصفية الباقات المجانية التي تحتوي على حرف f للقائمة المنسدلة
            final freeKeywordsList = _keywords.where((k) {
              final String kw = k['keyword'].toString().toLowerCase();
              final double p = (k['price'] as num?)?.toDouble() ?? 0.0;
              return kw.contains('f') || p == 0.0;
            }).toList();
            ...
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    // 🟢 زر الاختيار بين باقة مدفوعة أو مجانية
                    SegmentedButton<bool>(
                      segments: const [
                        ButtonSegment<bool>(value: false, label: Text('مدفوعة'), icon: Icon(Icons.attach_money)),
                        ButtonSegment<bool>(value: true, label: Text('مجانية (f)'), icon: Icon(Icons.card_giftcard)),
                      ],
                      selected: {isFree},
                      onSelectionChanged: (newSelection) {
                        setDialogState(() {
                          isFree = newSelection.first;
                          if (isFree) {
                            ...
                            // إلحاق حرف f تلقائياً إذا لم يكن موجهاً
                            if (!keywordController.text.toLowerCase().contains('f')) {
                              keywordController.text = '${keywordController.text}f';
                            }
                          }
                        });
                      },
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        // 🟢 إظهار حرف f ثابت داخل الحقل في حال المجاني فقط
                        suffixText: isFree ? 'f' : null,
                    ...
                    TextField(
                      ...
                      enabled: !isFree, // 👈 🟢 تعطيل الحقل إذا كانت الباقة مجانية
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      inputFormatters: [
                        FilteringTextInputFormatter.allow(RegExp(r'^\d*\.?\d*')),
                      ],
                      decoration: InputDecoration(
                        labelText: 'سعر الباقة / الفئة',
                        border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(16)),
                    ...
                    // 🟢 تغليف المفتاح بـ Opacity و IgnorePointer لتعطيله
                    Opacity(
                      opacity: isFree ? 0.5 : 1.0,
                      child: IgnorePointer(
                        ignoring: isFree, // 👈 تعطيل التفاعل
                        child: SwitchListTile(
                          contentPadding: EdgeInsets.zero,
                          title: const Text('تفعيل نظام المكافآت', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          subtitle: const Text('منح قسيمة مجانية عند الشراء التكراري'),
                          value: hasReward,
                    ...
                    if (hasReward) ...[
                      ...
                      DropdownButtonFormField<int?>(
                        ...
                        items: freeKeywordsList.map((k) { // 👈 🟢 التغيير هنا استخدام القائمة المفلترة
              ...
              actions: [
                TextButton(
                  ...
                  child:
                      Text('إلغاء', style: TextStyle(color: isDark ? Colors.grey.shade400 : Colors.grey.shade700)),
                ),
                ElevatedButton(
                  ...
                  onPressed: isSaving
                      ? null
                      : () async {
                          String rawText = keywordController.text.trim();
                          if (rawText.isEmpty) {
                            _showSnackBar('❌ الرجاء إدخال اسم/رمز الباقة', isError: true);
                            return;
                          }

                          // 🟢 إذا كانت مجانية ندمج حرف f مع الرقم تلقائياً
                          String keywordText = isFree ? '${rawText}f' : rawText;

                          // 🟢 ضبط قيم السعر والمكافآت بناءً على حالة isFree
                          double price = isFree ? 0.0 : (double.tryParse(priceController.text.trim()) ?? 0.0);
                          int targetCount = (hasReward && !isFree) ? (int.tryParse(targetCountController.text) ?? 0) : 0;
                          int rewardQty = (hasReward && !isFree) ? (int.tryParse(rewardQtyController.text) ?? 1) : 1;
                          ...
                          await _notifyNativeToReloadCache();
                          _loadData();
                        },
                  child: isSaving
                      ? const SizedBox(
                ...
                : _keywords.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          double price = (k['price'] as num?)?.toDouble() ?? 0.0;
