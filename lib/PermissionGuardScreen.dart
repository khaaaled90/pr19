...
class _PermissionsScreenState extends State<PermissionsScreen> with WidgetsBindingObserver {
  ...
  // إعادة الفحص التلقائي بمجرد الرجوع من إعدادات النظام إلى التطبيق
  @override
  ...
  Future<void> _checkAllPermissions() async {
    ...
    bool isListenerGranted = false;
    try {
      isListenerGranted = await _channel.invokeMethod('isNotificationListenerGranted') ?? false;
    } catch (_) {}

    // 4. فحص استثناء قيود البطارية
    final batteryStatus = await Permission.ignoreBatteryOptimizations.status;

    // 5. فحص صلاحية التشغيل التلقائي (Auto-Start / Battery Optimization)
    bool isAutoGranted = false;
    try {
      isAutoGranted = await _channel.invokeMethod('isAutoStartGranted') ?? false;
    } catch (_) {}
    ...
    setState(() {
      ...
      _notificationListenerGranted = isListenerGranted;
      _batteryOptimizationGranted = batteryStatus.isGranted;
      _autoStartGranted = isAutoGranted;
    });

    // الانتهاء والانتقال للواجهة الرئيسية عند اكتمال الصلاحيات
    if (_smsGranted &&
  ...
  // 4. توجيه لإعدادات التشغيل التلقائي
