      ...
      try {
        ...
        if (doc.exists && doc.data() != null) {
          ...
          if (!isActive) {
            ...
            // 🛑 إيقاف جميع خدمات الأندرويد والخلفية و SmsReceiver فوراً
            await _stopNativeBackgroundServices();
          ...
          // 🚨 إذا تبين بعد الفحص أن الترخيص منتهي برمجياً (تاريخ أو حد القسائم)
          if (localResult['isValid'] == false) {
            await _stopNativeBackgroundServices();
          } else {
            // 🟢 الترخيص ساري: إعادة تفعيل الخدمات إن كانت متوقفة
            await _enableNativeBackgroundServices();
          }
